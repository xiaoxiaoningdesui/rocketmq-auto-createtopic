...
public abstract class TopicRouteService extends AbstractStartAndShutdown {
    ...
    public MessageQueueView getAllMessageQueueView(ProxyContext ctx, String topicName) throws Exception {
        return getCacheMessageQueueWrapperV2(this.topicCache, topicName);
    }
    ...
    //remove static
    protected  MessageQueueView getCacheMessageQueueWrapperV2(LoadingCache<String, MessageQueueView> topicCache,
                                                              String key) throws Exception {
        MessageQueueView res = topicCache.get(key);
        if (res != null && res.isEmptyCachedQueue()) {
            try {
                String brokerAddress = "";
                ClusterInfo clusterinfo = this.mqClientAPIFactory.getClient().getBrokerClusterInfo(3000L);
                for (Map.Entry<String, BrokerData> entry : clusterinfo.getBrokerAddrTable().entrySet()) {
                    brokerAddress = entry.getValue().getBrokerAddrs().get(0L);
                }
                TopicConfig topicConfig = new TopicConfig();
                topicConfig.setTopicName(key);
                topicConfig.setWriteQueueNums(8);
                topicConfig.setReadQueueNums(8);
                topicConfig.setPerm(PermName.PERM_READ | PermName.PERM_WRITE);
                this.mqClientAPIFactory.getClient().createTopic(brokerAddress, TopicValidator.AUTO_CREATE_TOPIC_KEY_TOPIC, topicConfig, Duration.ofSeconds(3).toMillis());
                TopicRouteData topicRouteData = mqClientAPIFactory.getClient().getTopicRouteInfoFromNameServer(key, Duration.ofSeconds(3).toMillis());
                topicCache.put(key, buildMessageQueueView(key, topicRouteData));
                return topicCache.get(key);
            } catch (Exception error) {
                log.error("call getCacheMessageQueueWrapperV2 failed, ", error);
                throw new MQClientException(ResponseCode.GET_CACHE_MESSAGE_QUEUE_WRAPPER_V2,
                        "call getCacheMessageQueueWrapperV2 failed, topic: " + key);
            }
        }
        return res;
    }
